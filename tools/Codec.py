...
for i in range(1, max_inner + 1):
    # k を 4〜min(N//i, max_for) の全値で試算する
    # N//i と N//i+1 のみでは余りが再帰的にFOR化できるケースを見落とす
    # 例: N=68, max_inner=1 → k=34 (N//2) が最適だが N//1=68 しか見ない
...
while i < n:
    ...
    if len(data) == 1 and data[0] in SINGLE_BYTE_FRAME_OPS:
        ...
        while rem > 0:
            take = min(rem, 65)
            # バグ6修正: FOR禁止ルール repeat<4 を明確に記述
            # FOR(0xC0)=repeat2, FOR(0xC1)=repeat3 は生成禁止
            # → N<4 は直書きが正しい
            if take < 4:
                for k in range(take):
                    out.extend([op])
