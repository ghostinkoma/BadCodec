...
- bit_idx = y * width + x  (row-major, little-endian)
- ============================================================ */
...
BAD_STATIC_INLINE uint8_t  blk_nx(const bad_ctx_t *ctx) { return (uint8_t)(ctx->width  >> 3U); }
BAD_STATIC_INLINE uint8_t  blk_ny(const bad_ctx_t *ctx) { return (uint8_t)(ctx->height >> 3U); }
...
- Internal: scan-path coordinate generator
- 
- Pattern index (same as Python SCAN_PATHS):
- bit2=scan_dir (0=H 1=V)  bit1=start_y  bit0=start_x
- 
- Returns pixel coordinates in scan order via callback-style:
- Caller provides a pixel-setter; avoids a 64-entry coord table.
- 
- For RLE_BLOCK_4/8: pattern comes from opcode bits.
- ============================================================ */

/* Apply runs[] to gram block (bx,by) using scan pattern p_idx */
...
- Internal: frame-level fill / copy
- ============================================================ */
...
- 8 raw bytes, row-major, bit0=left pixel
- ============================================================ */
...
- 3 data bytes, 4 runs max, path[] based placement
- ============================================================ */
...
for (i = 0U; i < xor_len && bit_idx < BAD_BLOCK_PIXELS; i++) {
    ...
    for (k = 0U; k < run_len && bit_idx < BAD_BLOCK_PIXELS; k++, bit_idx++) {
        /* bit_idx maps to block pixel in row-major order */
        uint8_t py = bit_idx >> 3U;
        uint8_t px = bit_idx &  7U;
        uint16_t gi = blk_bit(ctx, bx, by, px, py);
        uint8_t pv  = gp_get(ctx->prev, gi);
        gp_set(ctx->gram, gi, pv ^ mask_v);
    }
}
return BAD_OK;
```
...
- Internal: fill / copy / invert one block
- ============================================================ */
...
- Writes directly to gram via scan-order loop.
- No intermediate pixel buffer needed -> AVR 2KB safe.
- ============================================================ */
...
if (scan_dir == 0U) {
    ...
    for (y=sy; dy>0?(y<(int16_t)h):(y>=0); y+=dy) {
        ...
        for (x=sx; dx>0?(x<(int16_t)w):(x>=0); x+=dx) {
            uint8_t c; NEXT_PX(c);
            gp_set(ctx->gram,(uint16_t)y*w+(uint16_t)x, c);
        }
    }
} else {
    int16_t x;
...
/* skip FRAME_DELIMITER(0x38) bytes and return the next opcode */
...
case BAD_OP_EXT_PREFIX:
    (void)bad_read1(ctx);   /* consume sub-command; skip */
    bad_memcpy(ctx->gram, ctx->prev,
               BAD_GRAM_SIZE(ctx->width, ctx->height));
    break;
...
- Public API: bad_result_str  (debug only - omit to save Flash)
- ============================================================ */
