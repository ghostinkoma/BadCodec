 ...
 */
  ...
  *  0x38 H/TL black  bit0=0 → 0  ✓
  *  0x39 H/TL white  bit0=1 → 1  ✓
  *  0x3A H/TR black  bit0=0 → 0  ✓
  *  0x3B H/TR white  bit0=1 → 1  ✓
  *  0x3D V/TL black  bit0=1 → must return 0  ← bit0 と逆
  *  0x3E V/TL white  bit0=0 → must return 1  ← bit0 と逆
  *  0x3D と 0x3E は opcode の bit0 が start_color と一致しないため
  *  専用の条件式で反転して返す。                               */
 #define BAD_RLE8_START_COLOR(op) \
     (((op)==0x3DU) ? 0U : ((op)==0x3EU) ? 1U : ((op)&0x01U))
 ...
 typedef struct {
     ...
     uint8_t      initialized;
     /* Frame-level FOR state (rev.19+)
      * bad_next_frame processes exactly 1 frame per call.
      * When a FOR(N)+inner_op is read from the stream, the first
      * iteration is processed immediately and the remaining N-1
      * iterations are stored here for subsequent calls.          */
     uint8_t      for_remain;   /* remaining repeat count (0 = not in FOR) */
     uint8_t      for_inner;    /* inner opcode being repeated             */
 } bad_ctx_t;
