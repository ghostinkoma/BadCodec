    ...
    with Pool(ncpu) as pool:
        for result in pool.imap_unordered(encode_frame_worker, tasks):
            ...
            while write_ptr in buffer:
                ...
                # ── アナライズ表示 (行数固定・残像なし) ──
                # 旧実装の問題:
                #   \033[H は画面(0,0)への絶対移動のみ。
                #   _block_map の返す行数が可変なので
                #   5行目以降に前フレームの残像が残った。
                # 修正:
                #   DISP_LINES = 出力行数を固定計算。
                #   \033[{N}A で正確にN行上へ移動。
                #   各行に \033[2K を付けて残像を消す。
                frame_no_display = write_ptr
                max_block_rows   = min(8, h // BLOCK_SIZE)
                DISP_LINES       = 4 + max_block_rows
                buf = []
                if write_ptr > st_i:
                    buf.append(f"\033[{DISP_LINES}A")
                else:
                    buf.append("\033[2J\033[H")
                buf.append(
                    f"\033[2K{C_BOLD}BadCodec v{APP_VERSION} Encoder"
                    f"{C_RESET}  [{ncpu} cores]  Phase 1/2: Encoding\n"
                )
                buf.append(f"\033[2K{_bar_block(p_rat)}\n")
                buf.append(
                    f"\033[2KFrame: {frame_no_display:04d} | "
                    f"Size: {len(bd):6d}B | "
                    f"Avg(90f): {avg:5.2f}% raw\n"
                )
                buf.append(f"\033[2K{'─' * 50}\n")
                block_lines = _block_map(bt, bx, max_rows=max_block_rows)
                for line in block_lines:
                    buf.append(f"\033[2K{line}\n")
                for _ in range(max_block_rows - len(block_lines)):
                    buf.append("\033[2K\n")
